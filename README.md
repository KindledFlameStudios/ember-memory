...
`ember-memory uninstall` is the safer cleanup helper: it removes the launcher, preserves your memory data by default, and shows the exact package uninstall command for your environment. The printed `pip uninstall -y` command removes the installed package without a second confirmation prompt. Use `ember-memory uninstall --delete-data` if you also want to remove `~/.ember-memory`.
