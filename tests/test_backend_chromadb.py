"""Tests for ChromaDB v2 backend.

All tests use tmp_path for fully isolated, in-process ChromaDB instances —
no external server required.

Vectors are 4-dimensional for speed. The dimension value is small enough that
ChromaDB's HNSW index operates without issue.
"""
...
def backend(tmp_path):
    """Return a connected ChromaBackendV2 with an isolated data directory."""
    b = ChromaBackendV2(data_dir=str(tmp_path))
    b.connect()
    return b
...
def _v(x=1.0, y=0.0, z=0.0, w=0.0) -> list[float]:
    """Return a 4-dimensional unit-ish test vector."""
    return [x, y, z, w]


def _setup_collection(backend: ChromaBackendV2, name: str = "test") -> None:
    backend.create_collection(name, dimension=4)


def _insert_one(
    backend: ChromaBackendV2,
    collection: str = "test",
    doc_id: str = "doc1",
...
def test_implements_memory_backend_interface():
    ...
    assert issubclass(ChromaBackendV2, MemoryBackend)
...
def test_create_collection_with_description(backend):
    ...
    # Description may be surfaced in list_collections
...
def test_search_returns_similarity_0_to_1(backend):
    ...
    _setup_collection(backend)
    # Use standard basis vectors (unit vectors) — guaranteed non-zero
    for i in range(4):
        vec = [0.0] * 4
        vec[i] = 1.0
...
def test_search_respects_limit(backend):
    ...
    for i in range(5):
        vec = [0.0] * 4
        vec[i % 4] = 1.0  # standard basis vectors cycling through 4 dims
...
# ── Backend factory (loader) ──────────────────────────────────────────────────


def test_get_backend_v2_returns_chromadb(tmp_path):
    from ember_memory.core.backends.loader import get_backend_v2
    b = get_backend_v2(backend="chromadb", data_dir=str(tmp_path))
    from ember_memory.core.backends.chromadb_backend import ChromaBackendV2
    assert isinstance(b, ChromaBackendV2)


def test_get_backend_v2_unknown_raises(tmp_path):
    from ember_memory.core.backends.loader import get_backend_v2
    with pytest.raises(ValueError, match="Unknown backend"):
        get_backend_v2(backend="nonexistent_backend", data_dir=str(tmp_path))


def test_get_backend_v2_already_connected(tmp_path):
    """Factory-returned backend should be immediately usable."""
    from ember_memory.core.backends.loader import get_backend_v2
    b = get_backend_v2(backend="chromadb", data_dir=str(tmp_path))
    b.create_collection("smoke", dimension=4)
    assert any(c["name"] == "smoke" for c in b.list_collections())
