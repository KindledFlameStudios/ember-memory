"""Tests for Qdrant v2 backend.

All tests use qdrant-client's in-memory mode — no external server required.

Vectors are 4-dimensional for speed. Cosine similarity scores are normalised
from Qdrant's native [-1, 1] range to [0, 1] via ``(score + 1) / 2``.
"""
...
@pytest.fixture
def backend():
    """Return a connected QdrantBackend using in-memory mode (no server needed)."""
...
def _setup_collection(backend: QdrantBackend, name: str = "test") -> None:
...
def test_implements_memory_backend_interface():
    ...
    assert issubclass(QdrantBackend, MemoryBackend)
...
def test_search_returns_similarity_0_to_1(backend):
    ...
    _setup_collection(backend)
    # Standard basis vectors — guaranteed non-zero, spans the full cosine range
    for i in range(4):
        vec = [0.0] * 4
        vec[i] = 1.0
...
def test_search_orders_by_descending_similarity(backend):
    ...
    # doc_partial is partially similar (45-degree angle)
    backend.insert("test", "doc_partial", "partial", _v(0.7, 0.7, 0.0, 0.0), {"k": "partial"})
    # doc_opposite points in the opposite direction
