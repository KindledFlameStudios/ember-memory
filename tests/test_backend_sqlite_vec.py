"""Tests for the SQLite-vec v2 backend.

All tests use tmp_path so each test gets a fresh, isolated .db file.
Vectors are 4-dimensional for speed.

If sqlite-vec is not installed, every test in this module is skipped
...
try:
    import sqlite_vec  # noqa: F401  — only needed to gate the tests
...
from ember_memory.core.backends.sqlite_vec_backend import SqliteVecBackend  # noqa: E402
...
def backend(tmp_path):
    """Return a connected SqliteVecBackend backed by a temp .db file."""
    b = SqliteVecBackend(db_path=str(tmp_path / "test.db"))
    b.connect()
    return b
...
def _setup_collection(backend: SqliteVecBackend, name: str = "test") -> None:
...
def test_implements_memory_backend_interface():
    ...
    assert issubclass(SqliteVecBackend, MemoryBackend)


def test_connect_required_before_use(tmp_path):
    """Calling any method before connect() must raise RuntimeError."""
    b = SqliteVecBackend(db_path=str(tmp_path / "unconnected.db"))
...
def test_create_collection_with_description(backend):
    backend.create_collection("annotated", dimension=4, description="test notes")
    cols = backend.list_collections()
    entry = next(c for c in cols if c["name"] == "annotated")
    assert entry.get("description") == "test notes"


def test_create_collection_exposes_dimension(backend):
    backend.create_collection("dimtest", dimension=8)
    cols = backend.list_collections()
    entry = next(c for c in cols if c["name"] == "dimtest")
    assert entry.get("dimension") == 8
...
def test_search_result_has_all_required_keys(backend):
    """Each search result dict must have id, content, metadata, similarity."""
    _setup_collection(backend)
    _insert_one(backend)
    results = backend.search("test", query_embedding=_v(1.0), limit=5)
    assert len(results) == 1
...
def test_get_preserves_metadata_types(backend):
    """Metadata round-trips through JSON correctly."""
    _setup_collection(backend)
    meta = {"count": 42, "score": 3.14, "active": True, "label": "hello"}
    backend.insert("test", "typed", "content", _v(1.0), meta)
    result = backend.get("test", "typed")
    assert result["metadata"]["count"] == 42
    assert abs(result["metadata"]["score"] - 3.14) < 1e-6
    assert result["metadata"]["active"] is True
    assert result["metadata"]["label"] == "hello"
...
def test_update_changes_search_results(backend):
    """After an update, searching should find the new vector."""
    _setup_collection(backend)
    # Insert doc pointing toward x-axis
    backend.insert("test", "doc1", "original", _v(1.0, 0.0, 0.0, 0.0), {})
    # Update it to point toward y-axis
    backend.update("test", "doc1", "updated", _v(0.0, 1.0, 0.0, 0.0), {})
    # Search along y-axis — should still find it
    results = backend.search("test", query_embedding=_v(0.0, 1.0, 0.0, 0.0), limit=1)
    assert len(results) == 1
    assert results[0]["id"] == "doc1"
    assert results[0]["content"] == "updated"
    # Similarity with y-axis query should be high (near 1.0)
...
def test_delete_removes_from_search(backend):
    """A deleted document must not appear in subsequent searches."""
    _setup_collection(backend)
    backend.insert("test", "keep", "keeper", _v(1.0, 0.0, 0.0, 0.0), {})
    backend.insert("test", "gone", "going away", _v(1.0, 0.0, 0.0, 0.0), {})
    backend.delete("test", "gone")
    results = backend.search("test", query_embedding=_v(1.0), limit=10)
...
def test_collection_peek_result_has_required_keys(backend):
    _setup_collection(backend)
    _insert_one(backend)
    results = backend.collection_peek("test")
    assert len(results) == 1
    assert {"id", "content", "metadata"} <= results[0].keys()
...
def test_data_persists_across_reconnect(tmp_path):
    """Data written in one session must survive closing and reopening the DB."""
    db_file = str(tmp_path / "persist.db")

    b1 = SqliteVecBackend(db_path=db_file)
    b1.connect()
    b1.create_collection("col", dimension=4)
    b1.insert("col", "persistent_doc", "I will survive", _v(1.0), {"stored": True})
    del b1  # closes the connection implicitly

    b2 = SqliteVecBackend(db_path=db_file)
    b2.connect()
    doc = b2.get("col", "persistent_doc")
    assert doc is not None
    assert doc["content"] == "I will survive"
    assert doc["metadata"]["stored"] is True


# ── Multiple collections ──────────────────────────────────────────────────────


def test_multiple_collections_are_isolated(backend):
    """Documents in different collections must not bleed into each other."""
    backend.create_collection("col_a", dimension=4)
    backend.create_collection("col_b", dimension=4)

    backend.insert("col_a", "a1", "in A", _v(1.0), {"col": "a"})
    backend.insert("col_b", "b1", "in B", _v(1.0), {"col": "b"})

    assert backend.collection_count("col_a") == 1
    assert backend.collection_count("col_b") == 1

    results_a = backend.search("col_a", query_embedding=_v(1.0), limit=10)
    assert all(r["metadata"]["col"] == "a" for r in results_a)

    results_b = backend.search("col_b", query_embedding=_v(1.0), limit=10)
    assert all(r["metadata"]["col"] == "b" for r in results_b)
